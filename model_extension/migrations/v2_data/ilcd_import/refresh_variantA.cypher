// ============================================================================
// refresh_variantA.cypher  --  recompute the Variant-A (real-dataset) EF3.1
// results after new ILCD packages were imported (ABS, PC, POM, PA6.6).
...
// round to 12 dp: trims float noise while preserving the small-magnitude EF3.1
// categories (particulate matter ~1e-8, ozone depletion ~1e-12, human tox ~1e-10)
// that round(_,6) previously flattened to 0.0
WITH efm, apl, base, art, ic, round(sum(mass_kg*perKg),12) AS value
...
// The MERGE above only ever adds/updates (artifact, category) pairs the current
// recompute yields; it never removes pairs that fell away (e.g. fossil-resource
// results after the LCI-hygiene unit split). Delete any EF3.1 Variant-A result
// whose category is no longer characterised by *any* contributing flow through
// the artifact's BOM. Declarations that REPORTS a pruned result simply lose that
// edge (03_declarations re-MERGEs REPORTS on its next run).
