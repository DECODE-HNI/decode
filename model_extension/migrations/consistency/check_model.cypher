// ============================================================================
// check_model.cypher   --  consistency review, Layer 2 (model / live DB)
// ----------------------------------------------------------------------------
// Read-only. Each statement returns one row: check id, a count, and the
// expected value. "n" <> "expect" (unless expect = 'info') is a finding.
// Run:  CQ_FMT=verbose ./cq.sh check_model.cypher
// ============================================================================
...
// A2: id-keyed labels only (HazardStatement is keyed on `code`, not a finding)
MATCH (n) WHERE size(labels(n)) > 0 AND n.id IS NULL
  AND NOT labels(n)[0] IN ['HazardStatement']
...
MATCH (x) WHERE x.scenarioRef IS NOT NULL
  AND NOT EXISTS { (ms:ModelScenario) WHERE ms.id = x.scenarioRef }
...
// B4: every Assessment needs ASSESSES + APPLIES_APPROACH + UNDER_SCENARIO;
// USES_METHOD only if it persists a result (8 approach-marker assessments are
// result-free and carry APPLIES_APPROACH only -- see fix_F02_assessment_edges).
MATCH (a:Assessment)
...
// ---- C. result completeness & value sanity -------------------------
...
// ---- D. cross-script agreement ------------------------------------
MATCH (art:Artifact)
WHERE EXISTS { (:Assessment {id:'ASSESS_EF31A_' + art.id}) }
  <> EXISTS { (:Assessment {id:'ASSESS_RECIPE_A_' + art.id}) }
RETURN 'D1 artifact has EF31A xor RECIPE_A assessment' AS check, count(art) AS n, '0' AS expect;

// generalised MAGNET finding: artifact whose non-PA12 part has a real dataset,
// yet EF3.1-A climate equals the value it would have from the PA12 interface alone
